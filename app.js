...
(() => {
  ...
  function setPhaseInfo(phase){
    // phase: {type, round}
    const base = phase.type === 'prep' ? 'Preparazione' : (phase.type === 'work' ? 'Work' : 'Rest');
    phaseLabel.textContent = base;
    if (phase.type === 'prep'){ roundLabel.hidden = true; }
    else { roundLabel.hidden = false; roundLabel.textContent = `ROUND ${phase.round}/${gym.rounds}`; }
  }
...
function setupWheel(container, list, max, onSelect){
  ...
  function snap(immediate=false){
    ...
    const y = container.scrollTop - spacer;
    const idx = Math.round(y / itemH);
    const value = Math.min(max, Math.max(0, idx));
    const targetTop = value * itemH + spacer;
    if (immediate){ container.scrollTop = targetTop; }
    else{
  ...
  container._scrollHandler = () => {
    ...
    state.scrollTimeout = setTimeout(()=>snap(false), 100);
  };
  ...
  container._keyHandler = (e) => {
    if (e.key === 'ArrowUp'){ e.preventDefault(); const v = Math.max(0, (container._value||0) - 1); container._value = v; const spacer=getSpacer(); container.scrollTop = v*itemH + spacer; onSelect(v); markSelected(list,v); }
    if (e.key === 'ArrowDown'){ e.preventDefault(); const v = Math.min(max, (container._value||0) + 1); container._value = v; const spacer=getSpacer(); container.scrollTop = v*itemH + spacer; onSelect(v); markSelected(list,v); }
  };
...
function animateGridCollapse(callback){
  ...
  burst.style.setProperty('--x', `${tx - 12}px`);
  burst.style.setProperty('--y', `${ty - 12}px`);
