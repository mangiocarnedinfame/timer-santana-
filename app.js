// app.js - fix selezione wheel (ricostruzione lists + rimozione/re-attach handlers)
(() => {
  // DOM elements
  ...
  // SVG radius math (safe guard if ring missing)
  const R = 88;
  const C = 2 * Math.PI * R;
  if (ring) {
  ...
  // ---------- build wheel with spacers so 0 and max can be centered ----------
  // returns the newly created UL (.list)
  function buildWheel(container, max){
    // remove any previous list to avoid duplicates
    ...
    // compute spacers to allow 0 and max to center
    requestAnimationFrame(() => {
      ...
      // create spacer elements (invisible) and insert
  ...
  // mark selected item in a given ul (.list), apply 3D transforms
  function markSelected(list, value){
    ...
    items.forEach(li => {
      ...
      // curve scaling adapted to big-touch (soft)
  ...
  // smooth scroll to the given value inside the container (assumes .list present)
  ...
  // attach wheel behavior on a container + associated list
  // removes previous handlers if present to avoid duplicates
  function setupWheelBehavior(container, list, max, onSelect){
    // remove old scroll handler if present
    ...
    const keyHandler = (e) => {
      ...
      if (key === 'ArrowUp' || key === 'PageUp'){
        ...
        scrollToValue(container, nxt);
      } else if (key === 'ArrowDown' || key === 'PageDown'){
        e.preventDefault();
        const nxt = Math.min(max, cur + 1);
        scrollToValue(container, nxt);
      } else if (key === 'Home'){
        e.preventDefault();
        scrollToValue(container, 0);
      } else if (key === 'End'){
        e.preventDefault();
        scrollToValue(container, max);
      }
    };
  ...
  // open/close picker handlers
  function openPicker(){
    overlay.hidden = false;
    // rebuild wheels to adapt to viewport and ensure correct spacers/list
    const rebuiltMin = buildWheel(minutesWheel, MAX_MIN);
    const rebuiltSec = buildWheel(secondsWheel, MAX_SEC);

    // re-attach behavior (removing previous handlers inside setupWheelBehavior)
  ...
  clockWrap.addEventListener('click', openPicker);
  clockWrap.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key===' ') { e.preventDefault(); openPicker(); } });
  ...
  function interpolateColor(pct){
    ...
    const g = hexToRgb('4de0a6');
    const y = hexToRgb('ffd166');
    const r = hexToRgb('ff6b6b');
    let c1, c2, t;
    if (pct > 0.5){
      t = (pct - 0.5) / 0.5;
      c1 = y; c2 = g;
  ...
  // initial visuals: mark whatever is present
  setTimeout(()=> {
  ...
  // re-request wake lock on visibility change when timer active
  ...
  // orientation / resize: if overlay is open, rebuild wheels to adapt sizes
