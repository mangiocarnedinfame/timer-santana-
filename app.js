...
function setupWheel(container, list, max, onSelect){
  ...
  function topSpacerH(){
    const sp = list.querySelector('.spacer');
    return sp ? sp.offsetHeight : 0;
  }

  function centerValue(){
    // Pick the item whose center is closest to the wheel's visual center
    const rect = container.getBoundingClientRect();
    const centerY = rect.top + rect.height/2;
    let sel = 0, best = Infinity;
    list.querySelectorAll('.wheel-item').forEach((it) => {
      const r = it.getBoundingClientRect();
      const y = r.top + r.height/2;
      const d = Math.abs(y - centerY);
      if (d < best){ best = d; sel = Number(it.dataset.value)||0; }
    });
    return Math.max(0, Math.min(max, sel));
  }

  function snap(immediate=false){
    const value = centerValue();
    const targetTop = topSpacerH() + value * ITEM_H;
    if (immediate){
  ...
  // Update selected while scrolling, then snap at rest
  container._scrollHandler = () => {
    const v = centerValue();
    if (v !== container._value){
      container._value = v;
      onSelect(v);
      markSelected(list, v);
    }
    if (state.scrollTimeout) clearTimeout(state.scrollTimeout);
    state.scrollTimeout = setTimeout(()=> snap(false), 120);
  };
  container.addEventListener('scroll', container._scrollHandler, {passive:true});

  // Ensure we snap also when the gesture ends quickly
  container._endHandler = () => snap(false);
  ['touchend','touchcancel','mouseup','pointerup','mouseleave','wheel'].forEach(ev => {
    container.addEventListener(ev, container._endHandler, {passive:true});
  });

  // Keyboard support
  container._keyHandler = (e) => {
    if (e.key === 'ArrowUp'){ e.preventDefault(); const v = Math.max(0,(container._value||0)-1); container._value=v; container.scrollTop = topSpacerH()+v*ITEM_H; onSelect(v); markSelected(list,v); }
    if (e.key === 'ArrowDown'){ e.preventDefault(); const v = Math.min(max,(container._value||0)+1); container._value=v; container.scrollTop = topSpacerH()+v*ITEM_H; onSelect(v); markSelected(list,v); }
  };
  ...
  function openPickerFor(ctx){
    ...
    if (ctx === 'rounds'){
      ...
      const itemH = 128;
      const spH = (list.querySelector('.spacer')?.offsetHeight) || 0;
      minutesWheel.scrollTop = spH + current*itemH;
